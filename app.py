...
.hero {background:linear-gradient(135deg,#0f2340 0%,#152f52 100%);color:white;padding:20px 24px;border-radius:16px;margin-bottom:16px;box-shadow:0 8px 24px rgba(15,35,64,.18);}
.hero h1 {margin:0;font-size:1.55rem;line-height:1.2;}
.hero p {margin:6px 0 0 0;color:#c9d6e4;font-size:0.9rem;line-height:1.4;}
.hero-meta {display:flex;flex-wrap:wrap;gap:8px;margin-top:10px;align-items:center;}
.hero-chip {display:inline-block;border:1px solid rgba(201,214,228,.45);border-radius:999px;padding:2px 9px;font-size:11px;color:#d6e3ef;}
.hero-badge {display:inline-block;border-radius:999px;padding:2px 9px;font-size:11px;font-weight:700;background:#dbeafe;color:#0f2340;}
.hero-creator {margin-left:auto;color:#9fb4ca;font-size:11px;text-align:right;white-space:nowrap;}
.demo-note {border:1px solid #d9e4f0;background:#f1f7ff;color:#3f566e;border-radius:10px;padding:8px 10px;margin-bottom:14px;font-size:0.8rem;}
.top-controls {display:flex;justify-content:flex-end;align-items:flex-start;padding-top:6px;}
.top-controls .stButton > button {width:auto;min-width:120px;border-radius:10px;border:1px solid #c8d2e0;background:#ffffff;color:#163453;font-size:0.82rem;padding:0.42rem 0.75rem;}
.card {position:relative;background:white;border:1px solid #e5e7eb;border-radius:14px;padding:14px 16px;box-shadow:0 3px 12px rgba(15,23,42,.07);min-height:195px;height:195px;display:flex;flex-direction:column;justify-content:flex-start;gap:5px;overflow:hidden;}
.card-accent {position:absolute;top:0;left:0;right:0;height:4px;}
.card-head {display:flex;align-items:center;gap:8px;}
.card-icon {width:24px;height:24px;border-radius:999px;background:#ecf2fb;color:#1f3f66;font-size:11px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;}
.card-title {font-size:12px;color:#526274;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:700;text-transform:uppercase;letter-spacing:.02em;}
.card-value {font-size:24px;font-weight:800;color:#0f172a;line-height:1.15;white-space:nowrap;}
.card-sub {font-size:12px;color:#64748b;margin-top:4px;display:flex;flex-direction:column;gap:3px;}
.card-delta {font-size:13px;font-weight:800;}
...
.section-shell {background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:14px 16px;box-shadow:0 2px 10px rgba(15,23,42,.05);margin-bottom:14px;}
.section-shell h3 {margin:0 0 8px;font-size:1rem;color:#17334f;display:flex;align-items:center;gap:8px;}
.section-icon {width:24px;height:24px;border-radius:999px;background:#eaf2ff;color:#1f3f66;font-weight:700;font-size:12px;display:inline-flex;align-items:center;justify-content:center;}
.app-footer {border-top:1px solid #e4ebf4;margin-top:8px;padding-top:10px;color:#5b6e82;font-size:0.78rem;line-height:1.5;}
.app-footer a {color:#2d5b88;text-decoration:none;}
...
if not dependency_registry.empty and changed_driver_values:
    st.markdown("<div class='section-shell'><h3><span class='section-icon'>DI</span>Business Dependency Intelligence</h3>", unsafe_allow_html=True)
    rows = []
...
accent_by_kpi = {
    "Revenue": "#3b82f6",
    "Gross Contribution": "#22c55e",
    "EBITDA": "#8b5cf6",
    "PAT": "#f59e0b",
    "Working Capital Requirement": "#14b8a6",
    "Bird Requirement": "#3b82f6",
    "Available Plant Capacity": "#22c55e",
    "CAPEX": "#8b5cf6",
    "OPEX": "#f59e0b",
    "Automation-adjusted HC": "#14b8a6",
}
for i, (icon, label, cur, scn, unit) in enumerate(metrics):
    delta = scn - cur
    delta_pct = None if cur in (None, 0) else ((scn - cur) / cur) * 100
    if "Working Capital" in label:
        status = "Needs Funding"
        status_color = "#0f766e" if delta > 0 else "#16a34a"
    elif "Bird" in label:
        status = "Procurement Increase"
        status_color = "#f59e0b" if delta > 0 else "#16a34a"
    elif "Capacity" in label:
        status = "Expansion Ready"
        status_color = "#0f766e" if delta > 0 else "#f59e0b"
    elif "CAPEX" in label:
    ...
    elif "Automation-adjusted HC" in label:
        status = "Efficiency Gain"
        status_color = "#16a34a" if delta < 0 else "#f59e0b"
    else:
        status = "Growing" if delta > 0 else "Softening" if delta < 0 else "Stable"
        status_color = "#16a34a" if delta > 0 else "#f59e0b" if delta < 0 else "#0f766e"
    ...
    with cols[i % 5]:
        st.markdown(f"""
        <div class="card">
          <div class="card-accent" style="background:{accent_by_kpi.get(label, '#3b82f6')};"></div>
          <div class="card-head">
...
with left:
    ...
    comp = pd.DataFrame(metrics, columns=["Icon", "KPI", "Current", "Scenario", "Unit"])
...
with col_a:
    ...
    table = pd.DataFrame({
        "KPI": [m[1] for m in metrics],
        "Current": [fmt(m[2], m[4]) for m in metrics],
        "Scenario": [fmt(m[3], m[4]) for m in metrics],
        "Delta": [fmt(m[3] - m[2], m[4]) for m in metrics],
        "Signal": ["Expansion Risk" if m[1] == "CAPEX" and capacity_util > 0.90 else ("Improves" if m[3] >= m[2] else "Weakens") for m in metrics]
    })
