...
from calculation_engine import align_manpower_sales, build_channel_sales_output, build_contract_pricing_output, build_distributor_output, build_explainability, build_financial_chain, build_logistics_output, build_order_capacity_intelligence, build_plant_planning, format_plant_configuration
...
.pbos-capacity-formula-title {
    font-size: 0.9rem;
    font-weight: 700;
    color: #334155;
    margin-bottom: 0.25rem;
}
.pbos-capacity-formula-line {
    font-size: 0.86rem;
    color: #475569;
    line-height: 1.45;
}
...
st.subheader("Business Strategic Planning")
st.caption("Scenario-based strategic planning for revenue, product portfolios, channels, plant capacity, procurement, logistics, manpower and profitability.")
...
def render_capacity_formula_block(title, lines):
    formula_lines = "".join(f"<div class='pbos-capacity-formula-line'>{escape(str(line))}</div>" for line in lines)
    st.markdown(
        f"""
        <div class='pbos-capacity-formula-card'>
          <div class='pbos-capacity-formula-title'>{escape(str(title))}</div>
          {formula_lines}
        </div>
        """,
        unsafe_allow_html=True,
...
def render_plant_capacity_governance_dialog():
    ...
    current_configuration_label = str(plant_capacity_output.get("current_configuration_label", format_plant_configuration(installed_lines, active_shifts)))
    ...
    st.markdown("<div class='pbos-capacity-dialog-content'>", unsafe_allow_html=True)
    ...
    required_fields_by_mode = {
        "CURRENT_SITE_OPTIMIZATION": [
            ...
            "recommended_capacity_mt_day",
        ],
        "NEW_PLANT_EXPANSION": [
            "maximum_site_lines",
            "maximum_site_shifts",
            "maximum_site_configuration_label",
            "maximum_current_plant_capacity_mt_day",
            "additional_capacity_required_mt_day",
            "recommended_new_plant_capacity_mt_day",
            "total_future_capacity_mt_day",
            "projected_future_utilization_pct",
        ],
    }
    if capacity_recommendation_mode in required_fields_by_mode:
        missing_fields = []
        for field in required_fields_by_mode[capacity_recommendation_mode]:
            field_value = plant_capacity_output.get(field)
            if field_value is None or (isinstance(field_value, str) and field_value.strip() == ""):
                missing_fields.append(field)
        if missing_fields:
            log_runtime_event(
                "plant_capacity_missing_canonical_fields_dialog",
                capacity_recommendation_mode=capacity_recommendation_mode,
                missing_fields=",".join(missing_fields),
            )
            st.warning("Configuration Review Required")

    displayed_lines = recommended_lines if capacity_recommendation_mode == "CURRENT_SITE_OPTIMIZATION" else maximum_site_lines
    displayed_shifts = recommended_shifts if capacity_recommendation_mode == "CURRENT_SITE_OPTIMIZATION" else maximum_site_shifts
    displayed_capacity = recommended_capacity if capacity_recommendation_mode == "CURRENT_SITE_OPTIMIZATION" else max_site_capacity
    expected_displayed_capacity = base_capacity * displayed_lines * displayed_shifts
    if abs(expected_displayed_capacity - displayed_capacity) > 1e-6:
        log_runtime_event(
            "plant_capacity_dialog_capacity_mismatch",
            capacity_recommendation_mode=capacity_recommendation_mode,
            displayed_lines=displayed_lines,
            displayed_shifts=displayed_shifts,
            displayed_capacity_mt_day=f"{displayed_capacity:.4f}",
            expected_capacity_mt_day=f"{expected_displayed_capacity:.4f}",
        )
        st.warning("Configuration Review Required")
...
def render_distributor_channel_drilldown(kpi_name):
    ...
    if kpi_name == "GT Sales Executives":
        ...
        render_responsive_detail_table("gt_sales_executives_detail", "GT Sales Executive Detail", pd.DataFrame([
            ...
            {"Item": "Primary workload", "Value": f"{gt['target_outlets']:,.0f} outlets"},
            {"Item": "Capacity per executive", "Value": f"{gt.get('effective_outlets_per_sales_executive', gt['outlets_per_sales_executive']):,.1f} outlets/executive"},
            {"Item": "Formula", "Value": gt.get("formula", "ceil(Target Outlets ÷ Outlets per Executive)")},
            {"Item": "Calculation", "Value": f"ceil({gt['target_outlets']:,.0f} ÷ {gt.get('effective_outlets_per_sales_executive', gt['outlets_per_sales_executive']):,.1f}) = {gt['sales_executives']:,.0f}"},
            ...
            {"Item": "Distributor managers required", "Value": f"{gt.get('distributor_managers', 0):,.0f}"},
            {"Item": "Manager formula", "Value": gt.get("manager_formula", "ceil(Distributors ÷ Distributors per Manager)")},
            ...
            {"Item": "Productivity assumption", "Value": f"Capacity multiplier {channel_sales_output.get('effective_capacity_multiplier', 1.0):,.2f}x"},
        ]))
    ...
    if kpi_name == "MT KAM":
        ...
        render_responsive_detail_table("mt_kam_detail", "Modern Trade KAM Detail", pd.DataFrame([
            ...
            {"Item": "Capacity per KAM", "Value": f"{data.get('accounts_per_kam', 0):,.1f} accounts/KAM"},
            {"Item": "Formula", "Value": data.get("formula", "ceil(Active MT Accounts ÷ Accounts per KAM)")},
            {"Item": "KAM required", "Value": f"{data['kam']:,.0f}"},
            {"Item": "Revenue context", "Value": fmt_currency(data["revenue"])},
        ]))
    ...
    if kpi_name == "QCom KAM":
        ...
        render_responsive_detail_table("qcom_kam_detail", "Quick Commerce KAM Detail", pd.DataFrame([
            ...
            {"Item": "Weighted workload", "Value": f"{data.get('weighted_workload', 0.0):,.1f}"},
            {"Item": "Capacity per KAM", "Value": f"{data.get('accounts_per_kam', 0):,.1f} workload units/KAM"},
            {"Item": "Formula", "Value": data.get("formula", "ceil(Weighted QCom Workload ÷ Accounts per KAM)")},
    ...
    if kpi_name == "HoReCa Sales HC":
        ...
        render_responsive_detail_table("horeca_sales_hc_detail", "HoReCa Sales HC Detail", pd.DataFrame([
            ...
            {"Item": "Active accounts", "Value": f"{data['accounts']:,.0f}"},
            {"Item": "Capacity per manager", "Value": f"{data.get('accounts_per_manager', 0):,.1f} accounts/manager"},
            {"Item": "Formula", "Value": "ceil(Active HoReCa Accounts ÷ Capacity per Manager)"},
            {"Item": "Required HoReCa HC", "Value": f"{data['sales_executives']:,.0f}"},
    ...
    if kpi_name == "Institutional / Government Manager":
        ...
        render_responsive_detail_table("institutional_government_manager_detail", "Institutional / Government Manager Detail", pd.DataFrame([
            ...
            {"Item": "Active Tenders / Contracts", "Value": f"{data['active_tenders_contracts']:,.0f}"},
            {"Item": "Capacity per manager", "Value": f"{data.get('tenders_per_manager', 0):,.1f} tenders/manager"},
            {"Item": "Formula", "Value": "ceil(Active Tenders ÷ Capacity per Manager)"},
            {"Item": "Required Institutional / Government HC", "Value": f"{data['required_hc']:,.0f}"},
            {"Item": "Product", "Value": data["product"]},
            {"Item": "Pack Size", "Value": f"{data['pack_size_kg']:,.0f} kg"},
            {"Item": "Contract Rate ₹/kg", "Value": f"{fmt_currency_plain(data['contract_rate_per_kg'], 2)}/kg"},
    ...
    if kpi_name == "Sales Manager":
        ...
        st.write("Governance rule: Head of Sales remains 1 while workload is within configured span-of-control.")
        st.write("CEO recommendation:")
...
st.subheader("Corporate Summary")
st.caption("Executive snapshot of revenue, demand, capacity and staffing.")
...
st.markdown("<div class='pbos-section-card'>", unsafe_allow_html=True)
st.subheader("Channel Business Ownership")
st.caption("Revenue ownership and accountable commercial leadership by channel.")
...
st.markdown("<div class='pbos-section-card'>", unsafe_allow_html=True)
st.subheader("Market-specific Planning")
st.caption("Selected market view with market-only presentation.")
...
st.markdown("<div class='pbos-section-card'>", unsafe_allow_html=True)
st.subheader("Commercial & Distribution Planning")
st.caption("Portfolio-wise route-to-market, distribution capacity and commercial coverage.")
...
st.markdown("#### A. Portfolio Distribution Network")
...
st.markdown("#### B. Portfolio Account & Network Capacity")
st.caption("Channel-account coverage, distributor capacity and route-to-market execution requirements.")
...
st.markdown("<div class='pbos-section-card'>", unsafe_allow_html=True)
st.subheader("Plant Capacity Planning")
st.caption("Capacity required by the plant to support assigned market demand.")
...
if capacity_recommendation_mode == "CURRENT_SITE_OPTIMIZATION" and abs(expected_recommended_capacity_mt_day - recommended_capacity_mt_day) > capacity_consistency_tolerance:
    ...
    third_card_subtitle = f"Expected {expected_recommended_capacity_mt_day:,.1f} MT/day from {format_plant_configuration(recommended_lines, recommended_shifts)}"
    st.warning("Configuration Review Required")
expected_max_capacity_mt_day = base_capacity * maximum_site_lines * maximum_site_shifts
if capacity_recommendation_mode == "NEW_PLANT_EXPANSION" and abs(expected_max_capacity_mt_day - max_capacity_mt_day) > capacity_consistency_tolerance:
    log_runtime_event(
        "plant_max_site_configuration_reconciliation_mismatch",
        expected_capacity_mt_day=f"{expected_max_capacity_mt_day:.4f}",
        reported_max_capacity_mt_day=f"{max_capacity_mt_day:.4f}",
    ...
    third_card_value = "Configuration Review Required"
    third_card_subtitle = f"Expected {expected_max_capacity_mt_day:,.1f} MT/day from {format_plant_configuration(maximum_site_lines, maximum_site_shifts)}"
    st.warning("Configuration Review Required")
...
if capacity_recommendation_mode in required_fields_by_mode:
    ...
    if missing_fields:
        log_runtime_event(
            "plant_capacity_missing_canonical_fields_main_card",
            capacity_recommendation_mode=capacity_recommendation_mode,
            missing_fields=",".join(missing_fields),
        )
        third_card_value = "Configuration Review Required"
        third_card_subtitle = f"Missing: {', '.join(missing_fields)}"
        st.warning("Configuration Review Required")
if capacity_recommendation_mode not in {"CURRENT_SITE_OPTIMIZATION", "NEW_PLANT_EXPANSION"}:
    log_runtime_event(
...
st.markdown("<div class='pbos-section-card'>", unsafe_allow_html=True)
st.subheader("Raw Material & Procurement Planning")
st.caption("Live bird procurement and raw material conversion for the selected market.")
...
st.caption("Primary live-bird movement and secondary finished-goods delivery.")
primary_logistics = logistics_output["primary"]
secondary_logistics = logistics_output["secondary"]
st.markdown("#### A. Primary Logistics")
st.caption("Live bird sourcing → plant")
...
st.markdown("#### B. Secondary Logistics")
st.caption("Plant → market / distributor / customer")
...
st.markdown("<div class='pbos-section-card'>", unsafe_allow_html=True)
st.subheader("Order & Capacity Intelligence")
st.caption("Actual order demand compared with planned volume, inventory and plant capacity.")
...
st.markdown("<div class='pbos-section-card'>", unsafe_allow_html=True)
st.subheader("Manpower Planning")
st.caption("Department-wise staffing for the selected plant and market plan.")
...
if manpower_output.get("staffing_bands"):
    ...
    for function_name, band in manpower_output["staffing_bands"].items():
        ...
        staffing_row = {
            ...
            "Current Workload": current_workload_display,
            "Unit": workload_unit,
            "Staffing Band": staffing_band,
            "Lower Threshold Numeric": lower_threshold_numeric,
            "Lower Threshold": lower_threshold_display,
            "Upper Threshold Numeric": upper_threshold_numeric,
        ...
        staffing_rows.append(staffing_row)
...
st.caption("Revenue bridge from procurement to EBITDA and PAT.")
...
st.markdown("<div class='pbos-section-card'>", unsafe_allow_html=True)
st.subheader("CEO Recommendation")
st.caption("Concise executive view for the current operating plan.")
